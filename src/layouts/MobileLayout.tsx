...
const THEME_LIST_PATH = "/themes/list";
...
export default function MobileLayout() {
  ...
  const isThemeList: boolean = pathname === THEME_LIST_PATH
  ...
  const hasHeader: boolean = isHome || isCommunity || isThemeList || isNotification || isMyPage
  const headerTitle: string = isCommunityWrite ? "글 작성" : isCommunityEdit ? "글 수정" : isHome ? "HOME" : isCommunity ? "커뮤니티" : isThemeList ? "테마 목록" : isNotification ? "알림" : isMyPage ? "마이페이지" : "고정 헤더"
  ...
  const hasBackArrow: boolean = pathSegments.length >= 2 && !isCommunityList && !isThemeList;
