...
export default function MobileLayout() {
  ...
  const isMyPage: boolean = pathname === "/mypage"
  const hasHeader: boolean = isHome || isCommunity || isMyPage
  const headerTitle: string = isCommunity ? "테마 커뮤니티" : isMyPage ? "마이페이지" : "고정 헤더"
  ...
  return (
    <div className="flex min-h-screen items-start justify-center bg-white py-4">
      <div className="relative flex h-[700px] w-[340px] flex-col overflow-hidden border border-secondary-200">
        ...
        <BottomTabBar isHome={isHome} isCommunity={isCommunity} isMyPage={isMyPage} />
      </div>
    </div>
  )
}
