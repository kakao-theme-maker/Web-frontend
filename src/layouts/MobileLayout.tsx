...
export default function MobileLayout() {
  ...
  const isDesignDetail: boolean = pathname.split('/').filter(Boolean).length >= 2 && isDesign && !isDesignBoardWrite
  const hasHeader: boolean = isHome || isCommunity || isDesign || isMyPage
  const headerTitle: string = isBoardWrite || isDesignBoardWrite ? "글 작성" : isCommunity ? "테마 커뮤니티" : isDesign ? "디자인 커뮤니티" : isProfileEdit ? "프로필 수정" : isMyPage ? "마이페이지" : "고정 헤더"
