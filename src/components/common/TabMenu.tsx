...
export default function TabMenu<T extends string>({ tabs, activeTab, onTabChange, cols = 2, hasInactiveBorder = false }: ITabMenuProps<T>) {
  return (
    <div className={cn("sticky top-0 z-10 grid bg-white text-center", GRID_COLS_CLASS[cols])}>
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <span
              className={cn("border-b-2 pb-1 px-1 transition-all", isActive ? "border-primary" : hasInactiveBorder ? "border-secondary-200" : "border-transparent")}
            >
