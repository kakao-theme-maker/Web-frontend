...
export function useThemeBoardDetail(boardId: number) {
  const { data, isLoading, isError } = useGetQuery(
    ['theme-board-detail', boardId],
    () => ThemeService.getThemeBoardDetail(boardId),
    { enabled: !!boardId },
  );

  const board: IThemeBoardDetail | null = data
    ? {
